...
volatile uint32_t* MR = &sw_pwm_timer.MR1; // base pointer to the match register MR1 - MR3, indexed MR[0-2]
...
[[gnu::optimize("O3")]] void TIMER3_IRQHandler(void) {
  ...
  const uint32_t interrupts = sw_pwm_timer.IR;
  sw_pwm_timer.IR = interrupts;  // clear all interrupts

  if (MCUI::util::bit_test(interrupts, 0)) {  // MR0 interrupt
    // reset all channels
    ...
    for(uint8_t MR_reg = 0; MR_reg < 3; MR_reg++) {
      if(next_pin != end_pin) {
        ...
        MR[MR_reg] = next_pin->match > sw_pwm_timer.TC + PWM_MATCH_OFFSET ? next_pin->match : sw_pwm_timer.TC + PWM_MATCH_OFFSET;
        MCUI::util::bit_set(sw_pwm_timer.MCR, MR_int[MR_reg]);     // enable this channels interrupt
  ...
  for(uint8_t MR_reg = 0; MR_reg < 3; MR_reg++) {
    if (MCUI::util::bit_test(interrupts, MR_reg + 1)) {
      ...
      if(next_pin != end_pin) {
        ...
        MCUI::util::bit_clear(sw_pwm_timer.MCR, MR_int[MR_reg]);
      }
    }
  }
  return;
