...
constexpr void bitset_set_value(Register& reg, const Value value, const Position position, const BitSet size) noexcept {
  auto tmp = reg & ~bitset_build_mask(position, size);
  reg = tmp | (value & bitset_build_mask(0, size)) << position;
}

template <typename Register, typename Value, typename Position, typename BitSet>
constexpr void bitset_set_write_only_value(Register& reg, const Value value, const Position position, const BitSet size) noexcept {
