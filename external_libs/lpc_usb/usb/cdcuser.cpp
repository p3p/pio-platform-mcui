 ...
 *---------------------------------------------------------------------------*/
void CDC_WrOutBuf() {
  if (CDC_OutContents <= LPC4078::usbcdc_buffer.receive_buffer.free()) {
    ...
    if(CDC_RecvCallback) {
      ...
      LPC4078::usbcdc_buffer.receive_buffer.write(buffer, bytesToWrite);
    }
...
void CDC_Init() {
  ...
  LPC4078::usbcdc_buffer.host_connected = false;
}
...
void CDC_Suspend() {
  _DBG("CDC Suspend\n");
  LPC4078::usbcdc_buffer.host_connected = false;
  LPC4078::usbcdc_buffer.transmit_buffer.clear();
}
...
void CDC_Resume() {
  _DBG("CDC Resume\n");
  LPC4078::usbcdc_buffer.host_connected = (CDC_LineState & CDC_DTE_PRESENT) != 0 ? true : false;
}
...
void CDC_BulkIn(void) {
  ...
  if (CDC_InContents != CDC_BUFFER_EMPTY) { _DBG("In Buffer busy\n"); }
    ...
    if (numBytesAvail > 0) {
      ...
      LPC4078::usbcdc_buffer.transmit_buffer.read(BulkBufIn, numBytesAvail);
