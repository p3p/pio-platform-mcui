...
uint32_t MSC_Acquire_Lock() {
  if (media_lock == DEVICE_LOCK) return 0;

  MCUCore::nvic_disable_irq(MCUI::IRQNumber::USB);
  device_wants_lock = true;
  uint32_t end_time = MCUI::time::millis() + 1000;
  if(media_lock == HOST_LOCK) {
    MCUCore::nvic_enable_irq(MCUI::IRQNumber::USB);
    while(media_lock == HOST_LOCK) {
      MSC_RunDeferredCommands();
      if(MCUI::util::elapsed(MCUI::time::millis(), end_time)) {
